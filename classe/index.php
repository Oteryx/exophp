...
<table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
        ...
        <?php
        function calculeage($calculeage){
            $age=date_create($calculeage)->diff(date_create('today'))->y;
            return $age;
        }
        $eleve=$dbconnexion->query('SELECT nom,prenom,date_naissance FROM users');
        $resultat=$eleve->fetchAll();
        foreach ($resultat as $value){
            echo '<tr><td>'.$value[0].'</td><td>'.$value[1].'</td><td>'.calculeage($value[2]).'</td></tr>';
        }
        ?>
